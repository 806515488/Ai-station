...
def tools() -> list[Tool]:
    ...
    return [
        Tool(name="remember", label="记住这件事",
             description="把一条应该在**以后每次对话**都记得的事实/偏好/口径记下来（跨会话长期有效）。"
                         "比如用户说'以后导出都用 A4''我不喜欢啰嗦的回答''无犯罪证明归十类'时用它。"
                         "只记长期有效的东西 —— 这次任务的过程、一次性的状态不要记。",
             run=t_remember,
        ...
        Tool(name="recall", label="回忆一下",
             description="检索长期记忆。回答用户问题前、或要做可能和以往约定冲突的事之前，先调它看看有没有相关约定。"
                         "不给 query 就列出全部记忆的索引。",
             run=t_recall,
