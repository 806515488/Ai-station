...
def _routing_hints(skill) -> str:
    ...
    if skill.id == "video":
        # 这一组**必须包含「视频」两个字**，因为草稿卡上的按钮预填的话术就是
        # "…生成视频吧" / "换个风格的视频提示词" —— 按钮也是一条路由输入，
        # 锚点里没有它，用户点了按钮会掉进通用聊天（那边没有 video.make）。09-13 踩过同类。
        return ("生成视频/做个视频/来一段视频/短视频/把这段描述生成视频/"
                "换个风格的视频提示词/视频好了吗/到哪了/放出来看看")
