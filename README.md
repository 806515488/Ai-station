...
- **260 个离线单测** —— 不联网、不烧 key、不碰真数据，全部可重复跑。
- **结构性护栏** —— 用 AST 查全仓，把「关键配置只有一个写入点」「所有 DB 访问必须持锁」这类约定钉死，而不是写在注释里靠人记。
- **变异验证** —— 关键护栏都验过「去掉它测试必须变红」，确保护栏本身不是摆设。
...
- [x] 宿主骨架 · agent 循环 · 技能注册表 · 异步任务 · 批准闸
- [x] 三层路由漏斗 · 渲染即工具 · 全局工具
- [x] Docker 化部署 · CI（编译检查 + 离线测试）
- [ ] 真卷大规模回归
- [ ] MCP 工具互操作
- [ ] 更多技能（生视频 / coding 助手 …）
