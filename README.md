...
- **模型只经一层适配** —— 换模型、接本地 Ollama，只动 `src/station/core/model.py`。
- **MCP 客户端** —— 接外部 MCP server 的工具。它们**不参与意图路由**：路由判的是「这句话属于哪个业务领域」，而「要不要搜索」是模型自己的判断 —— 这是两类工具，强行塞进路由，判词一失手工具就从模型手上消失了。MCP 工具直接进工具的常驻面，和内置全局工具并排。
  - **连不上不影响对话**：工具清单只在「测试连接」时拉一次并缓存，每轮组装零网络；对端挂了，也只是调它的那一刻才报错。
  - **安全边界**：用户能配的只有 `http` / `sse`（填个地址），**配不了 `stdio`** —— 那等于让服务器起任意进程，而这是个公网可达的站。内置服务才允许 stdio，且命令写死在代码里。
...
| `STATION_MCP_TIMEOUT` | 单次 MCP 调用最多等几秒（默认 30） |
...
├── tools/            全局工具（每个会话都注入）
├── mcp/              MCP 客户端（外部工具接入，含一个内置的网页搜索 server）
├── files/            产物区
